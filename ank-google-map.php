...
Plugin URI: http://github.com/ank91
Description: Simple and non-bloated WordPress Google Map Plugin. Written in pure javascript, no jQuery at all, responsive, configurable, no ads and 100% Free of cost.
Version: 1.0 (b)
Author: Ankur Kumar
Author URI: http://www.ankurkumar.hostreo.com
License: GPL2
*/
/*  Copyright 214  Ankur Kumar  (email : pro.ankurk1@gmail.com)
...
class Ank_Google_Map {
    ...
    function agm_add_color_picker() {
        if( is_admin() ) {
            ...
            // Add the color picker js  +css file
    ...
    function write_html_code(){
        ...
        echo '<div id="agm_map_canvas" style="width:'.$options["div_width"].$w_unit.';height:'.$options["div_height"].$h_unit.';margin: 0 auto;border:1px solid '.$options["div_border_color"].'"></div>';
    }
    ...
    function write_js_code(){
        ...
        $options = get_option('ank_google_map');
        $mapType='ROADMAP';
        if($options['map_type']==='1'){$mapType='ROADMAP';}elseif($options['map_type']==='2'){$mapType='SATELLITE';}elseif($options['map_type']==='3'){$mapType='HYBRID';}elseif($options['map_type']==='4'){$mapType='TERRAIN';}
        $marker_anim='BOUNCE';
        if($options['marker_on']==='1'){
           if($options['maker_anim']=='2'){$marker_anim='BOUNCE';}elseif($options['maker_anim']=='3'){$marker_anim='DROP';}
        }
        ?>
        <script src="//maps.googleapis.com/maps/api/js?sensor=false&amp;language=<?php echo $options['map_lang_code']?>"></script>
        <script>
            function loadMap(){
               ...
               center: cn, zoom: <?php echo $options['map_zoom'] ?>, mapTypeId: google.maps.MapTypeId.<?php echo $mapType;?>};
                var map = new google.maps.Map(agm, op);
                <?php if($options['marker_on']==='1') {?>
                var mk = new google.maps.Marker({position: cn, map:map <?php if($options['marker_anim']!=='1') { echo ", animation: google.maps.Animation.$marker_anim"; }?>, title:"<?php echo $options['marker_title'] ?>" });
                <?php  if($options['info_on']==='1') {?>
                var iw = new google.maps.InfoWindow({content: "<?php echo $options['info_text']?>"});
